...
short CalculateVacationDays(stDate VacationS, stDate VacationEnd)
{
	short VacationD = 0;
	while (IsDate1BeforeDate2(VacationS, VacationEnd))
	{
		if (IsBusinessDay(DayOrder(VacationS)))
			VacationD++;
...
int main()
{
	...
	cout << "Vacation From: " << DayNameByOrder(DayOrder(VacationS)) << " , " << VacationS.Day << "/" << VacationS.Month << "/" << VacationS.Year << endl; 
	cout << "Vacation To: " << DayNameByOrder(DayOrder(VacationEnd)) << " , " << VacationEnd.Day << "/" << VacationEnd.Month << "/" << VacationEnd.Year << endl; 
	short VacationDays = CalculateVacationDays(VacationS, VacationEnd);
	cout << "Actual Vacation Days is:" << VacationDays << endl;
	return 0;
}
